...
export default function Login() {
  ...
  return (
    <main className="min-h-screen pt-32 pb-20 grid md:grid-cols-2">
      ...
      <div className="flex items-center justify-center px-6 py-10">
        <div className="w-full max-w-md">
          ...
          <h1 className="headline-md mt-4 text-nest-char">{mode === "login" ? "Sign in" : "Join Nest"}</h1>
          ...
          <form onSubmit={submit} className="mt-8 space-y-3">
            {mode === "register" && (
              <>
                <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Full name" required className="w-full bg-white border border-nest-sand py-3 px-4 text-[14px]" data-testid="register-name-input" />
                <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Phone (optional)" className="w-full bg-white border border-nest-sand py-3 px-4 text-[14px]" data-testid="register-phone-input" />
              </>
            )}
            <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" placeholder="Email" required className="w-full bg-white border border-nest-sand py-3 px-4 text-[14px]" data-testid={mode === "login" ? "login-email-input" : "register-email-input"} />
            <input value={password} onChange={(e) => setPassword(e.target.value)} type="password" placeholder="Password" required className="w-full bg-white border border-nest-sand py-3 px-4 text-[14px]" data-testid={mode === "login" ? "login-password-input" : "register-password-input"} />
          ...
          <div className="mt-10 p-4 border border-nest-sand text-[12px] font-mono-sm text-nest-clay">
